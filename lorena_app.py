...
def _ensure_db():
    ...
    try:
        ...
        if not db_file.exists():
            log.info("Banco não encontrado em %s — inicializando...", db_path)
            db_init.init_db()
            db_init.seed_default_instructions()
            log.info("Banco inicializado com sucesso.")
        else:
            # Garante schema atualizado mesmo se o arquivo já existe
            db_init.init_db()
...
app.add_url_rule(
    "/internal/api-explore",
    endpoint="api_explore",
    view_func=_wh.api_explore,
    methods=["GET"],
)
